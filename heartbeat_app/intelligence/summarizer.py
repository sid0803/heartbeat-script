...
def _build_prompt(events: List[Any], is_daily: bool, preferences: str, source_errors: List[str] = None) -> str:
    if is_daily:
        intro = (
            ...
            "[CALENDAR] YESTERDAY IN ONE LINE: <single sentence summary>\n\n"
            "🏆 TOP WIN: <best thing that happened>\n\n"
            "[CRITICAL] UNRESOLVED RISKS: <bullet list -- anything still open>\n\n"
        ...
        intro = (
            ...
            f"Founder preferences: {preferences}\n\n"
            "Your ONLY job is to convert customer, deal, meeting, and delivery signals into a clear decision brief.\n"
            ...
            "[NOTE] BOTTOM LINE: [ONE sentence -- what the founder should focus on right now]\n"
        )
        if source_errors:
            intro += "\nDATA GAPS (some founder context may be missing):\n"
...
class Summarizer:
    ...
    def summarize(self, events: List[Dict[str, Any]], is_daily: bool = False, source_errors: List[str] = None) -> str:
        ...
        if not events and not is_daily:
            if source_errors:
                return f"All clear on visible customer and meeting signals. {len(source_errors)} source(s) need reconnection for full coverage."
            return "All clear. No customer, revenue, or meeting risks need action right now."
        ...
        for provider in self._provider_order():
            if not self._key_for(provider):
                continue
            try:
                print(f"[BRAIN] Generating {'DAILY' if is_daily else 'QUICK'} digest with {provider.upper()}...")
                result = self._call_provider(provider, prompt)
                print(f"[OK] Digest generated by {provider.upper()}.")
                return result
            except ImportError:
                print(f"[!]  '{provider}' library not installed. Trying next provider...")
            except Exception as e:
                print(f"[!]  {provider.upper()} error: {e}. Trying next provider...")

        # ── Rich mock fallback ------------------------------------------------
        print("[INFO]  No AI keys configured. Using MOCK digest.")
        if is_daily:
            return (
                "[CALENDAR] **DAILY EXECUTIVE SUMMARY (MOCK)**\n"
                "[CRITICAL] Critical Items:\n"
                "   • Client ABC waiting for reply since 12 hrs\n"
                "   • Renewal agenda not confirmed for tomorrow's client call\n"
                "[NOTE] Recommended Actions:\n"
                "   1. Reply to Client ABC on Slack\n"
                "   2. Assign proposal owner and confirm renewal prep\n"
                "⏱️ Next Check-in: Evening standup"
            )

        mock_error_tag = f"\nDATA GAP: {source_errors[0]}" if source_errors else ""
        return (
            "[CRITICAL] **ACTION REQUIRED:**\n"
            "   1. Reply to Client XYZ's deadline question within 2 hours\n"
            "   2. Assign a clear proposal owner before the next buyer follow-up\n\n"
            "[WARNING] **FOR AWARENESS:**\n"
            "   • Renewal call needs pricing notes and agenda confirmation\n\n"
            "[OK] **ALL CLEAR:**\n"
            "   • No other customer or meeting risks need action right now\n\n"
            "[NOTE] **BOTTOM LINE:** Protect the active deal by replying to Client XYZ and preparing the renewal agenda."
            f"{mock_error_tag}"
        )
